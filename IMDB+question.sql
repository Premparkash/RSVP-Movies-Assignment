...
FROM (
  SELECT g.genre, COUNT(DISTINCT m.id) AS movie_count, RANK() OVER (ORDER BY COUNT(DISTINCT m.id) DESC) AS genre_rank
  FROM movie AS m
...
top_directors AS (
  ...
  JOIN top_3_genres tg ON tg.genre = g.genre
...
FROM names AS n
JOIN role_mapping AS rm ON rm.name_id = n.id
...
(
  SELECT m.id, m.title, m.year, m.country, r.avg_rating, r.total_votes
  FROM movie AS m
...
(
  ...
  INNER JOIN india_movies AS im ON rm.movie_id = im.id
...
actor_ratings AS
(
  SELECT ia.name AS actor_name,
         SUM(im.total_votes * im.avg_rating) AS weighted_rating_sum,
         SUM(im.total_votes) AS total_votes_sum,
         COUNT(im.id) AS movie_count
  FROM india_movies AS im
  INNER JOIN role_mapping AS rm ON im.id = rm.movie_id
  INNER JOIN indian_actors AS ia ON rm.name_id = ia.id
  WHERE rm.category = 'actor'
  GROUP BY ia.name
),
ranked_actors AS
(
  SELECT actor_name,
         total_votes_sum,
         movie_count,
         weighted_rating_sum / total_votes_sum AS actor_avg_rating,
         RANK() OVER (ORDER BY weighted_rating_sum / total_votes_sum DESC, total_votes_sum DESC) AS actor_rank
  FROM actor_ratings
)
SELECT actor_name, total_votes_sum, movie_count, actor_avg_rating, actor_rank
FROM ranked_actors
ORDER BY actor_rank
...
FROM names n
JOIN role_mapping rm ON rm.name_id = n.id AND rm.category = 'actress'
JOIN movie m ON m.id = rm.movie_id AND m.country = 'India' AND m.languages LIKE '%Hindi%'
...
hits AS (
    SELECT mm.production_company, COUNT(*) AS movie_count
    FROM multilingual_movies mm
    INNER JOIN ratings r ON r.movie_id = mm.id
    WHERE r.median_rating >= 8
    GROUP BY mm.production_company
    ORDER BY COUNT(*) DESC
    LIMIT 2
)
SELECT production_company, movie_count, DENSE_RANK() OVER (ORDER BY movie_count DESC) AS prod_comp_rank
FROM hits;
